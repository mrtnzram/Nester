...
class SongAnalysis(object):
    ...
    def export_to_json(self, file_name, onsets_seconds, offsets_seconds, bout_duration_seconds, ms_per_pixel):  # kts added 1/22/24
        try:
            ...
            # Extract species and bird ID from the file name
            parts = os.path.basename(file_name).split('_')
            if len(parts) < 3:
                raise ValueError(f"Unexpected file naming format: {file_name}")

            species_and_id = parts[1]
            species_parts = species_and_id.split('-')
            if len(species_parts) < 2:
                raise ValueError(f"Unexpected species and ID format: {species_and_id}")

            species = f"{species_parts[0]}-{species_parts[1]}"
            bird_id = species_parts[2] if len(species_parts) > 2 else "unknown"
            bout_info = parts[-1].split('.')[0]  # Assuming bout info is the last part before the extension

            # Construct processed_file_name
            processed_file_name = f"SegSyllsOutput_{species_and_id}_{parts[2]}_{bout_info}.json"
            
            # Use json_out for the output directory
            if self.json_out:
                json_file_path = os.path.join(self.json_out, processed_file_name)
            else:
                json_file_path = os.path.splitext(file_name)[0] + ".json"

            # Construct wav_location dynamically
            parent_dir = os.path.dirname(file_name)  # Get parent directory
            corrected_dir = parent_dir.split("SegSyllsOutput")[0].rstrip("\\")  # Truncate after "SegSyllsOutput"
            wav_file_name = processed_file_name.replace("SegSyllsOutput_", "").replace(".json", ".wav")
            wav_location = os.path.join(corrected_dir, wav_file_name)

            # Create JSON data using processed_file_name
            json_data = {
