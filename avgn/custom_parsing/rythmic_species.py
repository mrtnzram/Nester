...
def generate_json(row, DT_ID):
    try:
        ...
        json_dict = {
            ...
            "species": species,
            "id": row['id'],
            "wav_loc": str(row['wavloc']),  # Updated key
            "samplerate_hz": sr,
            "length_s": wav_duration,
            "boutid": row['boutid']
        }
        
         # Format JSON name as 'species_birdid_boutid'
        json_name = f"{species.lower().replace(' ', '_')}_{row['id']}_{row['boutid']}"
        json_out = (
