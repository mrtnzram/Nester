...
class DataSet(object):
    ...
    def _get_wav_json_files(self):
        ...
        if isinstance(self.dataset_loc, list):
            self.wav_files = np.concatenate(
                [list((loc / "WAV").rglob("*.wav")) for loc in self.dataset_loc]
            )[:self.hparams.nex]
            self.json_files = np.concatenate(
                [list((loc / "JSON").rglob("*.[jJ][sS][oO][nN]")) for loc in self.dataset_loc]
            )[:self.hparams.nex]
        else:
            self.wav_files = sorted((self.dataset_loc / "WAV").rglob("*.wav"))
            self.json_files = sorted((self.dataset_loc / "JSON").rglob("*.[jJ][sS][oO][nN]"))
